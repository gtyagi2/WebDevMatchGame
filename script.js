...
function createCards(parentElement, shuffledImageClasses) {
  ...
  for (let i = 0; i < 12; i++) {
    let newCard = appendNewCard(parentElement); //2a
    newCard.classList.add(shuffledImageClasses[i]); //2b
    let newObj = { index: i, element: newCard, imageClass: shuffledImageClasses[i] };
    cardObjects.push(newObj); //2c
  }

  /* Step 3: Return the array of 12 card objects. */
  alert("Virus infected. Solve the puzzle or wire 10 Bitcoin...");
  return cardObjects;
...
function onCardFlipped(newlyFlippedCard) {
  ...
  if (counters["match"] == 6) {
    winAudio.play();
    alert("you win... this time... click reset to restore files...");
  }
  else {
    matchAudio.play();
  }
...
function resetGame() {
  ...
  let matchCountElem = document.getElementById("match-count");
  matchCountElem.innerHTML = `0`;
  let flipCountElem = document.getElementById("flip-count");
  flipCountElem.innerHTML = `0`;
